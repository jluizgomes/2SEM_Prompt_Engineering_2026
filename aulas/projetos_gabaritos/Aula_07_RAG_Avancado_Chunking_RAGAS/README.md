...
Chunking semântico compatível com LangChain 1.x, ParentDocumentRetriever e reranking com cross-encoder, além de avaliação RAGAS quando as dependências e credenciais externas estão disponíveis.
...
A avaliação RAGAS executa o mesmo fluxo de recuperação e geração usado na demonstração. Quando a API ou as credenciais externas não estão disponíveis, o código informa a limitação e os testes usam doubles injetados; nenhuma métrica é simulada.
...
python -m compileall -q main.py server.py tests
```

Os testes usam doubles em memória e não acessam Ollama, Hugging Face, Chroma persistente ou a rede.
...
```bash
./rodar.sh          # prepara o ambiente e sobe em http://127.0.0.1:8107
