...
export default function App() {
  ...
  async function executar(endpoint, corpo, rotulo, streaming = false) {
    setCarregando(true);
    const id = Date.now();
    setResultados((r) => [...r, {
      id,
      rotulo: rotulo || endpoint,
      resposta: { tipo: 'texto', conteudo: '' },
    }]);
    try {
      const resposta = streaming
        ? await api.stream(endpoint, corpo, (parte) => {
            setResultados((r) => r.map((item) => {
              if (item.id !== id) return item;
              const atual = item.resposta?.tipo === 'texto' ? String(item.resposta.conteudo || '') : '';
              return {
                ...item,
                resposta: {
                  ...item.resposta,
                  tipo: 'texto',
                  conteudo: atual + String(parte.conteudo || ''),
                },
              };
            }));
          })
        : await api.post(endpoint, corpo);
      setResultados((r) => r.map((item) => (
        item.id === id ? { ...item, resposta: respostaTipo(resposta) } : item
      )));
      ...
      setResultados((r) => r.map((item) => (
        item.id === id ? { ...item, resposta: { tipo: 'erro', conteudo: String(e) } } : item
      )));
      return null;
    } finally {
      setCarregando(false);
  ...
  function submitAcao(ev, acao) {
    ...
    executar(acao.endpoint, corpo, acao.titulo || acao.endpoint, Boolean(acao.streaming));
  }
    ...
    <div className="app">
      ...
      {info.implementado === false && (
        <div className="banner incompleto">
          ⚠️ Este exercício não foi carregado. Verifique o servidor e as dependências.
        </div>
      )}
      {(info.avisos || []).map((a, i) => (
