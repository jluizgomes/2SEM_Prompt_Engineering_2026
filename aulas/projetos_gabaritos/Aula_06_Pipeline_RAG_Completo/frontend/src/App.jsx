...
export function FontesView({ fontes = [] }) {
  if (!Array.isArray(fontes) || fontes.length === 0) return null;
  return (
    <details className="fontes-chat">
      <summary>Fontes recuperadas ({fontes.length})</summary>
      <ol>
        {fontes.map((fonte, indice) => (
          <li key={fonte.id || `${fonte.fonte}-${indice}`}>
            <strong>{fonte.fonte || 'Documento'}</strong>
            {fonte.pagina != null && <span> · página {fonte.pagina}</span>}
            {fonte.trecho && <p>{fonte.trecho}</p>}
          </li>
        ))}
      </ol>
    </details>
  ...
  async function enviarChat(ev) {
    ...
    try {
      ...
      if (resposta && resposta.tipo === 'erro') {
        ...
        const fontes = (resposta && (resposta.fontes || (resposta.extra && resposta.extra.fontes))) || [];
        setMensagens((m) => [...m, { papel: 'assistente', texto, passos, fontes }]);
      }
      if (resposta && resposta.aprovacao_pendente) {
        setAprovacao({
          endpoint: info.aprovacao_endpoint || '/api/aprovar',
    ...
    <div className="app">
      ...
      {info.implementado === false && (
        <div className="banner incompleto">
          Este exercício ainda está em configuração. Consulte as ações disponíveis.
        </div>
      )}
      {(info.avisos || []).map((a, i) => (
