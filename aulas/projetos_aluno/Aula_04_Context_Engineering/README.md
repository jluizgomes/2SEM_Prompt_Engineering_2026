...
Contagem de tokens com tiktoken e montagem de prompt com instrução de sistema + pergunta.
...
```bash
cd Aula_04_Context_Engineering
...
```bash
./rodar.sh          # prepara o ambiente e sobe em http://127.0.0.1:8004
# (Windows: .\rodar.ps1) — passo a passo manual:
#   python3 -m venv .venv && source .venv/bin/activate
#   pip install -r requirements.txt
#   cd frontend && npm install && npm run build
#   python -m uvicorn server:app --port 8004
```
