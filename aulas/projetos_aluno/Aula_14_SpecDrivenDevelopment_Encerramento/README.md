...
Escrever a especificação primeiro e deixar o modelo gerar o artefato conforme o spec.
...
```bash
cd Aula_14_SpecDrivenDevelopment_Encerramento
...
```bash
./rodar.sh          # prepara o ambiente e sobe em http://127.0.0.1:8014
# (Windows: .\rodar.ps1) — passo a passo manual:
#   python3 -m venv .venv && source .venv/bin/activate
#   pip install -r requirements.txt
#   cd frontend && npm install && npm run build
#   python -m uvicorn server:app --port 8014
```
