...
classificador = classificador_prompt | llm.with_structured_output(Intencao)
...
chain_documentos = (
    {"contexto": vectorstore.as_retriever(search_kwargs={"k": 2}),
     "pergunta": RunnableLambda(lambda x: x["pergunta"])}
...
def rotear(entrada: dict) -> str:
    intencao = classificador.invoke({"pergunta": entrada["pergunta"]})
    categoria = intencao.categoria.lower()
    if categoria == "documentos":
        return chain_documentos.invoke(entrada)
    if categoria == "calculo":
        return chain_calculo.invoke(entrada)
    return chain_geral.invoke(entrada)
...
def main() -> None:
    ...
    for pergunta in [
        ...
        print(f"P: {pergunta}")
        print(f"R: {router.invoke({'pergunta': pergunta})}\n")
