...
class AddRequest(BaseModel):
    ...
    # False grava a mensagem literal, sem pedir ao LLM para extrair fatos.
    # Útil para testes e para quando o modelo local é pequeno demais para
    # devolver o JSON de extração de forma confiável.
    infer: bool = True
...
def _com_escopo(metodo, **kwargs):
    """Chama um método do mem0 tolerando as duas APIs de escopo.

    A partir do mem0ai 2.x os parâmetros de entidade (user_id, agent_id,
    run_id) não são mais aceitos no nível raiz de search() e get_all():
    precisam ir em filters={...}. Versões antigas só aceitam o formato
    anterior, então tentamos o novo e caímos no antigo.
    """
    escopo = kwargs.pop("escopo")
    try:
        return metodo(filters=escopo, **kwargs)
    except ValueError as exc:
        if "filters" not in str(exc):
            raise
        return metodo(**escopo, **kwargs)
...
def list_memories(
    ...
    try:
        return _com_escopo(
            get_memory().get_all,
            limit=limit,
            escopo=_scope(user_id, agent_id, run_id),
        )
    except HTTPException:
        raise
    except Exception as exc:  # noqa: BLE001
...
def search(req: SearchRequest) -> Dict[str, Any]:
    try:
        return _com_escopo(
            ...
            query=req.query,
            limit=req.limit,
            escopo=_scope(req.user_id, req.agent_id, req.run_id),
        )
    except HTTPException:
        raise
    except Exception as exc:  # noqa: BLE001
