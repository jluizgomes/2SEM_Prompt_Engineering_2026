...
Modelos padrão baixados: `qwen3.5:0.8b` (610 MB) e `qwen3-embedding:0.6b` (990 MB); nos perfis `memory` e `complete`, mais `llama3.2:1b` (1,3 GB), além dos modelos listados em `EXTRA_MODELS`. Cada modelo extra de 7–8 B custa **4 a 6 GB** a mais.
...
Só isso. O alvo `up` chama `setup` antes (cria o `.env` a partir do `.env.example`, gera todos os secrets locais, garante a pasta `models/` e mantém `.gguf` opcional), constrói as três imagens locais, sobe o perfil `complete` e imprime as URLs. Se `models/` estiver vazio, o Ollama registra um aviso, tenta obter os modelos públicos configurados e mantém o container no ar; a montagem do volume não falha.

**O primeiro boot é lento e isso é normal.** São dois tempos somados: o build das imagens (5 a 15 min) e o download dos modelos do Ollama (~2,9 GB sem `EXTRA_MODELS`; cresce conforme a lista configurada). Enquanto o modelo não terminar de baixar, a interface abre mas o chat responde erro. Acompanhe:
...
| `fiap-ollama-cpu:latest` | `./ollama` | Entrypoint que importa **todos** os `.gguf` de `models/` e baixa `EMBEDDING_MODEL`, guardrails e `EXTRA_MODELS` no boot |
...
make up-memory
make up-crawl
make up-langfuse          # observabilidade isolada
make up-complete-langfuse # complete + Langfuse
make validate-langfuse    # containers, OTLP, sessions, conexão e score configs
...
# 2. suba (o make RECUSA subir se a ENCRYPTION_KEY estiver com o
#    placeholder troque-por-openssl-rand-hex-32)
make up-langfuse

# 3. o primeiro boot roda as migrações — leva alguns minutos
make logs-langfuse

# 4. se precisar recriar/atualizar a conexão do Playground
#    (ex.: depois de make pull M=algum-modelo)
make langfuse-ollama
```

O `make up-langfuse` **espera todos os containers saudáveis, valida os secrets, registra a LLM connection do Ollama e executa o teste integral do Langfuse**. O comando só termina com sucesso depois de provar a ingestão OTLP, o agrupamento de sessions, a conexão do Playground e os três score configs. Para uma stack já ativa, use `make validate-langfuse`.
...
Ollama no Playground e no LLM-as-a-judge: aqui entram **duas peças independentes**, e ambas já vêm prontas:

1. **Permissão de rede (env var).** O Langfuse **bloqueia hosts de rede privada** por padrão, e `ollama` é um deles — sem isso a conexão morre com *"Blocked IP address detected"*. É o papel do `LANGFUSE_LLM_CONNECTION_WHITELISTED_HOST=ollama`, já no compose (variável `LANGFUSE_WHITELISTED_HOST` no `.env`).
2. **A LLM Connection em si (dado no projeto).** É ela que diz "adapter openai, baseURL `http://ollama:11434/v1`, estes modelos" — fica no Postgres do Langfuse e **não tem variável de ambiente de bootstrap**. Sem criar, o Playground abre mas não tem *nenhum* modelo na lista. O `make up-langfuse` espera as migrações e a cria sozinho via `scripts/langfuse_ollama_setup.py` (upsert idempotente); para recriar ou atualizar a lista de modelos: **`make langfuse-ollama`**.
...
### 8.3 Pipelines: memória, Firecrawl e Langfuse
...
| `langfuse_trace_filter.py` | `filter` | **Langfuse Trace** — em *Admin → Pipelines*, aplicado a todos os modelos |
...
Ele faz o scrape em markdown, corta em `MAX_CHARS` e manda para o Ollama responder. As valves (URL base, modelo, limite de caracteres) são editáveis em *Admin → Pipelines*. O modelo default segue a env `CHAT_MODEL` do container — se trocar no `.env`, a pipeline acompanha.

**Langfuse Trace** complementa o OTEL do backend (`ENABLE_OTEL=true`, §8.4): o OTEL traça a **infra** (HTTP/DB e as chamadas ao Ollama); a pipeline grava o **nível de negócio** — o prompt do aluno, a resposta e o `usage` do Ollama — como observação `GENERATION` no Langfuse (OTLP/HTTP, com `session`, `user` e `model`). Se o Langfuse não estiver no ar a pipeline falha em silêncio e o chat continua. Valves: `ENABLED`, `LANGFUSE_BASE_URL`, chaves pk/sk (default das env do container).

### 8.4 Observabilidade Open WebUI → Langfuse (OTEL)

Com o perfil `langfuse` no ar e `WEBUI_OTEL=true` no `.env`, o Open WebUI exporta **traces de infraestrutura** (FastAPI, SQLAlchemy, Redis, HTTP ao Ollama…) e **métricas** para o Langfuse via OTLP/HTTP — as mesmas chaves `LANGFUSE_PUBLIC_KEY/SECRET_KEY` usadas pelo filtro acima. `ENABLE_OTEL_LOGS` fica `false` porque a Langfuse v4 não expõe OTLP de logs.

Ativação: `make setup` (o `.env` já sai com `WEBUI_OTEL=true`) e `make up-langfuse` (ou `make up`). Para desligar: `WEBUI_OTEL=false` no `.env` e recriar o container (`docker compose --profile complete --profile langfuse up -d open-webui-full`).

> Sem o Langfuse no ar o chat **não quebra** — só não há exportação.
...
#### Sessions — agrupando a conversa inteira

Sem session, cada `chain.invoke()` vira um trace solto na lista. Com
`sessionId`, o Langfuse agrupa todos os traces de uma mesma conversa e
oferece o **session replay** (ver
[docs](https://langfuse.com/docs/observability/features/sessions)).
É a mesma ideia do `session_id` da memória por histórico — só que aqui
quem separa é o **Langfuse**, não a chain.

Com **LangChain** (a chave é o metadata `langfuse_session_id`):
...
chain.invoke(
    ...
    config={
        "callbacks": [CallbackHandler()],
        "metadata": {"langfuse_session_id": f"aluno-{matricula}"},
    },
)
# invokes seguintes com o MESMO matricula caem na mesma session
```

Com o **SDK v3 puro**, propague o `session_id` para todas as
observações filhas com `propagate_attributes`:

```python
from langfuse import propagate_attributes

with propagate_attributes(session_id=f"aluno-{matricula}"):
    chain.invoke({"tema": "embeddings"}, config={"callbacks": [handler]})
```

Por baixo (OTLP direto, como o `integration_test.py` faz) o atributo é
`langfuse.session.id` em **cada span** — no v4 a API REST antiga de
sessions saiu, mas o agrupamento continua: mesma `sessionId` ⇒ mesma
session na UI (menu **Sessions** em http://localhost:3001). Use um
identificador estável por conversa (matrícula, ticket, `uuid`), não um
por invoke — trace com sessionId diferente a cada chamada é trace solto
com outro nome.
...
| `CHAT_MODEL` | `MiniCPM5-2B-...-Q4_K_M` | Modelo padrão da interface (vem do GGUF em `models/`) |
| `EMBEDDING_MODEL` | `qwen3-embedding:0.6b` | Embeddings do RAG (1024 dimensões) |
| `GGUF_FILE` / `GGUF_MODEL` | `MiniCPM5-...gguf` | Modelo **principal** importado no boot; todos os `.gguf` de `models/` entram (nome = arquivo sem `.gguf`) |
| `MODELS_DIR` | `./models` | Pasta no host montada em `/models` (read-only) |
| `EMBEDDING_CONTEXT` | `512` | Contexto da variante enxuta de embeddings |
| `OLLAMA_KEEP_ALIVE` | `-1` | `-1` nunca descarrega o modelo (resposta instantânea) |
| `OLLAMA_CONTEXT_LENGTH` | `8192` | Janela de contexto dos modelos |
| `OLLAMA_NUM_PARALLEL` / `OLLAMA_MAX_LOADED_MODELS` | `1` / `2` | Concorrência e modelos residentes |
| `OLLAMA_GPU_COUNT` | `all` | GPUs reservadas para o Ollama |
| `EXTRA_MODELS` | `nomic-embed-text-v2-moe,nomic-embed-text,embeddinggemma` | Modelos extras, separados por vírgula |
...
| `LANGFUSE_WHITELISTED_HOST` | `ollama` | Permissão de rede anti-SSRF; a LLM Connection do Playground é criada pelo `make langfuse-ollama` |
...
```bash
make test-unit       # bootstrap, pasta models/, secrets e validações do Langfuse
make health          # endpoints e serviços ativos
make test            # integração ponta a ponta, vista do SEU computador
make test-net        # o mesmo, de dentro da rede fiap-ai-net
make validate-langfuse # validação estrita e completa do Langfuse
```
...
- Open WebUI `/health`
- ChromaDB heartbeat
- Pipelines expondo `mem0_memory_filter`, `firecrawl_web` e `langfuse_trace_filter`
...
- Langfuse: `/api/public/health` (prova que as migrações terminaram), um trace de verdade gravado por OTLP em `/api/public/otel/v1/traces` (prova o caminho web → MinIO → Redis → worker → ClickHouse), **2 traces agrupados sob a mesma session** via `langfuse.session.id` (prova a feature Sessions), a LLM connection do Ollama (prova que o Playground e o LLM-as-a-judge têm modelo — sem `make langfuse-ollama` este teste falha) e os **3 score configs** da aula (`qualidade-resposta`, `aderencia-instrucao`, `classificacao-aula`)
...
up-complete-langfuse  complete + Langfuse
check-langfuse-env     valida secrets, portas, whitelist e OTEL
validate-langfuse      valida containers, traces, sessions, conexão e scores
langfuse-ollama (re)cria a LLM connection do Ollama no Langfuse
...
test-unit       Testes locais de bootstrap, configuração e Langfuse
...
São dois problemas diferentes, com sintomas diferentes:

1. **Erro "Blocked IP address detected"** ao salvar a conexão: é a validação anti-SSRF. Confira `LANGFUSE_WHITELISTED_HOST=ollama` no `.env` e reinicie o Langfuse (`docker compose --profile langfuse up -d`) — a env var só vale depois de recriar os containers. A URL base tem de ser `http://ollama:11434/v1` (hostname interno, não `localhost`).
2. **Playground abre mas a lista de modelos está vazia:** falta a LLM Connection no projeto. Rode:

```bash
make langfuse-ollama
```

O `make up-langfuse` já faz isso no primeiro subida; o alvo acima é para quando você subiu sem make, apagou a connection na mão, ou baixou um modelo novo (`make pull M=...`) e quer ele aparecer no seletor. Confira com `make health` — a linha *"Langfuse ↔ Ollama"* tem de dizer `OK`.
...
├── ollama/
│   ├── Dockerfile
│   └── entrypoint.sh           # importa TODOS os .gguf de models/ e baixa os demais modelos no boot
│
├── models/                     # arquivos .gguf locais (montados em /models; todos importados)
...
│
└── scripts/
    ├── integration_test.py       # teste ponta a ponta, sem dependências
    ├── langfuse_ollama_setup.py  # cria a LLM connection do Ollama (make langfuse-ollama)
    └── searxng_engines.py        # corrige os engines do SearXNG (make searxng-engines)
```
